...
class MultiHeadedMLPModule(nn.Module):
    ...
    def __init__(self,
                 n_heads, #In MLP, n_heads is 2 (q and v heads)
                 input_dim, #input_dim is the number of states, bus engine replacement problem, it is 1 (mileage)
                 output_dims, #if [3,2,4], then 3 heads with output_dims 3, 2, 4
                 hidden_sizes,
                 hidden_nonlinearity=torch.nn.ELU(),
    ...
    def forward(self, input_val):
        ...
        """
        x = input_val #dimension is (batch_size, horizon)
