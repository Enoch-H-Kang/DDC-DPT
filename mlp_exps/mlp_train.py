...
def train(config):
    if not os.path.exists('mlp/figs/loss'):
        os.makedirs('mlp/figs/loss', exist_ok=True)
    if not os.path.exists('mlp/models'):
        os.makedirs('mlp.models', exist_ok=True)
    if not os.path.exists('mlp/logs'):
    ...
    for epoch in tqdm(range(config['num_epochs']), desc="Training Progress"):
        ...
        for i, batch in enumerate(train_loader):
            ...
            #loss = ce_loss + 0.001* be_loss
            loss = ce_loss + loss_ratio(epoch, 0, config['loss_ratio'], 500) *be_loss
            #loss = torch.log(ce_loss)+torch.log(loss_ratio(epoch, 0.001, config['loss_ratio'], 100)*be_loss)
