...
def build_log_filename(config):
    ...
    filename = (f"{config['env']}_num_trajs{config['num_trajs']}"
                f"_dummies{config['num_dummies']}x{config['dummy_dim']}"
                f"_beta{config['beta']}_theta{config['theta']}"
                f"_H{config['H']}_ratio{config['loss_ratio']}"
    ...
    for epoch in tqdm(range(config['num_epochs']), desc="Training Progress"):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(test_loader):
                ...
                last_states = states[:,-1,:] #dimension is (batch_size, state_dim)
            ...
            if epoch_Q_MSE_loss < best_Q_MSE_loss:
                ...
                best_normalized_true_Qs = last_true_Qs
                best_normalized_pred_q_values = last_pred_q_values
                best_epoch = epoch    
        ...
        for i, batch in enumerate(train_loader): #For batch i in the training dataset
            ...
            if i %2 == 0: # update model paramters only
                ...
                TQmQ = config['beta']*(vnext_reshaped.clone().detach() - chosen_vnext_values_reshaped)
                predicted_rho_naive = chosen_vnext_values_reshaped*config['beta'] + pivot_rewards_reshaped
            ...
            if i == 0: #i=0 means the first batch
                pred_r_values = pred_q_values[:,-1,:] - config['beta']*pred_vnext_values[:,-1,:]
                states = batch['states']
                last_states = states[:,-1,:] #dimension is (batch_size, state_dim)
                pred_r_values_with_states = torch.cat((last_states, pred_r_values), dim=1) #dimension is (batch_size, state_dim+action_dim)
        ...
        if (epoch + 1) % 1 == 0:
                ...
                plt.subplot(6, 1, 1) # Adjust to 5x1 grid
                plt.yscale('log')
                plt.xlabel('epoch')
                ...
                # Plotting BE loss
                plt.subplot(6, 1, 2) # Second plot in a 5x1 grid
                plt.yscale('log')
                plt.xlabel('epoch')
                plt.ylabel('BE Loss')
                ...
                # Plotting CE loss
                plt.subplot(6, 1, 3) # Third plot in a 5x1 grid
                plt.yscale('log')
                plt.xlabel('epoch')
                plt.ylabel('CE Loss')
                ...
                # Plotting Q MSE loss
                plt.subplot(6, 1, 4) # Fourth plot in a 5x1 grid
                plt.yscale('log')
                plt.xlabel('epoch')
                ...
                plt.subplot(6, 1, 5) # Fifth plot in a 5x1 grid
                plt.yscale('log')
                plt.xlabel('epoch')
                plt.ylabel('expV Loss')
                plt.plot(test_vnext_MSE_loss[1:], label="Test V(s',a') MSE Loss", color='orange')
                plt.legend()
                
                # Plotting D loss (V(s') - E[V(s')] loss)
                plt.subplot(6, 1, 6) # Fifth plot in a 5x1 grid
                plt.yscale('log')
                plt.xlabel('epoch')
                plt.ylabel('D Loss')
                plt.plot(train_D_loss[1:], label="D Loss", color='purple')
                plt.legend()
