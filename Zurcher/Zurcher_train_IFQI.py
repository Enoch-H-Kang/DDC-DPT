        ...
        for epoch in tqdm(range(config['num_epochs']), desc="Training Progress"):
            ...
            for i, batch in enumerate(train_loader): #For batch i in the training dataset
                ...
                while convergence == False:
                    pred_q_values, pred_q_values_next, r_values = model(batch) #dimension is (batch_size, horizon, action_dim)
                    #r values are negative, as they are costs
                    #TD error = r(s,a) + beta*V(s',a') - Q(s,a)
                    TD_error = r_values + config['beta']*q_values - q_values
