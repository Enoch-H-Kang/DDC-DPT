...
def CCPandTKEst(data):
    ...
    #The instruction says matrix form, but I commented out that part and implemented dictionary form instead to deal with sparce matrix
    #Matrix form code runs well also. Youo can uncomment them and run them.

    #Given mileage and decision data d, estimate CCPs and transition kernel P nonparametrically (frequency counts)
    #CCPs = np.zeros((len(s), 2))
    CCPs = {}
    #CCPsCount = np.zeros((len(s), 2)) #CCPsCount is a matrix of counts of the occurances of each state-action pair
    #P = np.zeros((len(s), len(s)))
    #PCount = np.zeros((len(s), len(s))) #PCount is a matrix of counts of the occurances of each state-state pair
    ...
    for i in range(data.shape[0]):
        ...
        choicestorage[current_state][data.iloc[i, 4]] += 1   
            #PCount[current_state, next_state] += 1
    #Estimate CCPs and P by normalizing the counts
    #Calculate the CCPs and P by normalizing the counts in choicestorage and transitionstorage
    for key in choicestorage:
        CCPs[key] = choicestorage[key] / np.sum(choicestorage[key])
    #total_transitions_from_state = {}
    #for (current_state, _), count in transitionstorage.items():
    #    if current_state not in total_transitions_from_state:
    #        total_transitions_from_state[current_state] = 0
    #    total_transitions_from_state[current_state] += count

    #for (current_state, next_state), count in transitionstorage.items():
    #    P[(current_state, next_state)] = count / total_transitions_from_state[current_state]

    #CCPs = np.nan_to_num(CCPsCount / np.sum(CCPsCount, axis=1)[:, None]) #CCPs is a matrix of probabilities of each state-action pair
    ...
    for state in choicestorage:
        for stepsize in range(1, 5):  # Transitions to state+1, state+2, state+3, state+4
            ...
            if next_state in CCPs: 
                ...
                for action in [0,1]: 
                    llh += choicestorage[state][action] * np.log(Prob[action]) /4
    #print(llh)
    return -llh
...
#mileage = data_new.iloc[:, 3].values 
